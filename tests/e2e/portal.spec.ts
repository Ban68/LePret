...
test.describe("portal cliente", () => {
  ...
  test("extracción de factura desde PDF devuelve datos normalizados", async ({ request }) => {
    test.skip(!orgId, "Define E2E_ORG_ID en tu entorno para validar la extracción de facturas.");
    test.skip(
      !storageStatePath,
      "Define PLAYWRIGHT_STORAGE_STATE con una sesión válida para invocar el extractor de facturas.",
    );
