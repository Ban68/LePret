  ...
  const handleWizardSubmit = async () => {
    ...
    if (wizardData.targetRate && wizardData.targetRate.trim().length > 0) {
      ...
      if (!Number.isFinite(normalizedRate)) {
        errors.targetRate = "Ingresa una tasa válida";
    ...
    try {
      ...
      if (!res.ok || !data?.ok || typeof data.url !== "string") {
        const friendly = data?.error === 'file_not_found'
          ? "Aún no has cargado un soporte para esta solicitud."
      ...
      const msg = e instanceof Error ? e.message : "Error descargando soporte";
      setErr(msg);
      toast.error(msg);
    } finally {
      setBusy(false);
