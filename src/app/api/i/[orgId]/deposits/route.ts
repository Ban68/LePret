...
import { createNotification, notifyInvestorDepositRequested } from "@/lib/notifications";
...
export async function POST(req: Request, { params }: RouteContext) {
  try {
    ...
    try {
      await notifyInvestorDepositRequested({
        orgId,
      ...
      console.error("Failed to notify investor about contribution request", notifyError);
    }
    ...
    notifyStaff({
      ...
      console.error("Failed to notify staff about contribution request", notifyError);
    });
...
async function notifyStaff(params: {
  ...
  await createNotification(staffIds, "investor_transaction_request", "Nuevo aporte pendiente", {
    orgId: params.orgId,
